...
RUN apt-get update && apt-get install -y  git-core libcurl4-openssl-dev libgit2-dev libssh2-1-dev libssl-dev libxml2-dev make pandoc pandoc-citeproc zlib1g-dev libbz2-dev liblzma-dev && rm -rf /var/lib/apt/lists/*
RUN echo "options(repos = c(CRAN = 'https://bioconductor.org/packages/3.11/bioc', CRAN = 'https://cran.rstudio.com', CRAN = 'https://bioconductor.org/packages/3.11/data/annotation', CRAN = 'https://bioconductor.org/packages/3.11/data/experiment', CRAN = 'https://bioconductor.org/packages/3.11/workflows'), download.file.method = 'libcurl')" >> /usr/local/lib/R/etc/Rprofile.site
...
RUN Rscript -e 'remotes::install_version("liftOver",upgrade="never", version = "1.12.0")'
